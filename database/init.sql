...
CREATE TABLE IF NOT EXISTS orders (
  ...
  status ENUM('in_progress', 'submitted', 'completed', 'cancelled') DEFAULT 'in_progress' COMMENT '状态: in_progress-服务中, submitted-已提交待居民确认, completed-已确认结算, cancelled-已取消',
  service_hours DECIMAL(8, 2) DEFAULT 0 COMMENT '实际服务时长(小时)',
  start_time DATETIME COMMENT '开始时间',
  submitted_at DATETIME COMMENT '志愿者提交服务结果时间',
  confirmed_at DATETIME COMMENT '居民确认结算时间',
  reject_reason VARCHAR(500) COMMENT '居民退回原因',
...
CREATE TABLE IF NOT EXISTS reviews (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uk_order_reviewer (order_id, reviewer_id) COMMENT '每方对同一订单只能评价一次',
